...
def cache_options(object_ids, wechatuser):
    ...
    judgements_status = Judgement.objects.filter(article__id__in=object_ids, wechatuser=wechatuser).values('id', 'article_id', 'judge', 'remind_date', 'hint_text')
    status_set = {}
    map(lambda x: status_set.update({x['article_id']: x['judge']}), judgements_status)
    cache_set('{0}_{1}'.format(ARTICLE_JUDGEMENT_STATUS_KEY, wechatuser.openid), status_set)
    
    judge_set = {}
    map(lambda x: judge_set.update({x['article_id']: {'remind_date': x['remind_date'], 'hint_text': x['hint_text'], 'id': x['id']}}), judgements_status)
    cache_set('{0}_{1}'.format(ARTICLE_JUDGEMENT_DATE_KEY, wechatuser.openid), judge_set)
